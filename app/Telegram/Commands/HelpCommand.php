...
class HelpCommand extends UserCommand
{
    ...
    public function execute(): ServerResponse
    {
        ...
        if ($command_str === '') {
            $text = 'Для выгрузки фото просто отошлите их боту, в ответ бот пришлет вам ссылку.'.PHP_EOL;
            ...
            if ($safe_to_show && count($admin_commands) > 0) {
                $text .= PHP_EOL . '*Список админских команд*:' . PHP_EOL;
                foreach ($admin_commands as $admin_command) {
                    $text .= '/' . $admin_command->getName() . ' - ' . $admin_command->getDescription() . PHP_EOL;
                }
            }

            $text .= PHP_EOL . 'Для помощи по конкретной команде: /help <command>';
        ...
        return $this->replyToChat('Не найдено описание: Команда `/' . $command_str . '` не найдена', ['parse_mode' => 'markdown']);
    }
